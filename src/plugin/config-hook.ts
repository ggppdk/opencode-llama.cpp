...
import {getLlamaCppProviderEntries, validateConfig} from '../utils/validation'
...
export function createConfigHook(client: PluginInput['client'], toastNotifier: ToastNotifier) {
    ...
    // const initialModelCount = config?.provider?.['llama.cpp']?.models ? Object.keys(config.provider['llama.cpp'].models).length : 0
        ...
        if (getLlamaCppProviderEntries(config).length === 0) {
    ...
    const discoveryPromise = enhanceConfig(config, client, toastNotifier)
        ...
        const finalModelCount = llamaCppProviders.reduce((count, [, providerConfig]) => {
            const models = providerConfig?.models
            return count + (models && typeof models === 'object' ? Object.keys(models).length : 0)
        }, 0)

        if (finalModelCount === 0 && llamaCppProviders.length > 0) {
